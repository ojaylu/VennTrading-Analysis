...
print("Accoding to Oscar's method, the buy and sell signal will be ", combineflag)
...
history, test = train_test_split(data['close'], test_size=0.3, random_state=432, shuffle=True)
print(len(history))
print(len(test))
...
for t in range(len(test)):
    ...
    signals = np.append(signals, signal[0])
    obs = test[t]
    history=np.append(history,obs)
...
x_future = model_fit.forecast(500, alpha = 0.05)
x_future = pd.DataFrame(x_future)
print(x_future)
