...
def bruteforce(data, plotgraph,plotmacdgraph,plotrsigraph,plotobvgraph,plotpsargraph, plotemagraph):
    ...
    for i, spl in enumerate(strat_params_list):
        ...
        psar_buy_price, psar_sell_price = psar(data, plotpsargraph=plotpsargraph)
        ...
        newPSARPriceBuy = [item for item in psar_buy_price if not(math.isnan(item)) == True]
        newPSARPriceSell = [item for item in psar_sell_price if not(math.isnan(item)) == True]
        ...
        if plotgraph == True:
            ...
            plotPSARPriceBuy = [0 if math.isnan(x) else x for x in psar_buy_price]
            plotPSARPriceSell = [0 if math.isnan(x) else x for x in psar_sell_price]
            ...
            plt.subplot(2, 5, 9)
            plt.plot(plotPSARPriceBuy, label = "psar buying price")
            plt.subplot(2, 5, 10)
            plt.plot(plotPSARPriceSell, label = "psar selling price")
            plt.xlabel('time')
            plt.ylabel('price')
            ...
            plt.legend()
            plt.show()
        surplus = sum(newMACDPriceSell) + sum(newRSIPriceSell) + sum(newOBVPriceSell) + sum(newEMAPriceBuy) + sum(newPSARPriceBuy)
        - sum(newMACDPriceBuy) - sum(newRSIPriceBuy) - sum(newOBVPriceBuy) - sum(newEMAPriceSell) - sum(psar_sell_price)
        ...
        df1 = pd.DataFrame({'macd buying price':[macd_buy_price],
                              ...
                              'psar buying price':[psar_buy_price],
                              'psar selling price':[psar_sell_price],
                              'surplus':surplus})
        ...
        if surplus >= highestsurplus:
            ...
            PSARstrategy = psar_buy_price, psar_sell_price
    ...
    print("the optimized PSAR strategy is strategy",strategyID,"Below is the strategy trading details ", PSARstrategy)
    ...
    MACDstrategy, RSIstrategy, OBVstrategy, EMAstrategy, PSARstrategy,combineflag, data, newsurplus, flagsummary = flagrules(
        data, MACDstrategy, RSIstrategy, OBVstrategy, EMAstrategy, PSARstrategy ,flagstrategy)
