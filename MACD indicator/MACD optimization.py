...
def MACD():
    ...
    def implement_stoch_macd_strategy_optimization_surplus(prices, k, d, macd, macd_signal, lb, ub):    
        buy_price = []
        sell_price = []
        stoch_macd_signal = []
        signal = 0
        ...
        newsignPriceBuy = [item for item in buy_price if not(math.isnan(item)) == True]
        newsignPriceSell = [item for item in sell_price if not(math.isnan(item)) == True]
        #print(newsignPriceBuy, newsignPriceSell)
        surplus =  sum(newsignPriceSell) - sum(newsignPriceBuy)
        return surplus

    lb = list(range(45,50,1))
    ub = list(range(50,55,1))
    strat_params_list = list(product(lb, ub))
    strat_params_dict_list = {'ub': [40, 50, 60],'lb': [45, 55, 65]}
    surplus = 0
    ...
    for i, spl in enumerate(strat_params_list):
         ...
         #buy_price, sell_price, stoch_macd_signal = implement_stoch_macd_strategy_optimization(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1])
         #print(buy_price, sell_price, stoch_macd_signal)
         if implement_stoch_macd_strategy_optimization_surplus(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1]) > surplus:
            surplus = implement_stoch_macd_strategy_optimization_surplus(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1])
            x = implement_stoch_macd_strategy_optimization(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1])
            strategyID = i
            print("This strategy help you earn",implement_stoch_macd_strategy_optimization_surplus(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1]))
         else:
            print("This strategy help you earn",implement_stoch_macd_strategy_optimization_surplus(data['close'], data['%k'], data['%d'], data['macd'], data['macd_signal'], strat_params_list[i][0], strat_params_list[i][1]))
