...
def rsi_buy_sell(signal, col1, col2, cashbalance, borrowed):
    signPriceBuy = []
    signPriceSell = []
    flag = -1
    #shareNo = 0
    ...
    for i in range(0, len(signal)):
        ...
        if (signal[col1][i] == 1 and signal[col2][i] == 0) and flag != 1:
            shareNo = 10000
            if cashbalance >= signal['close'][i] * shareNo:
                signPriceBuy.append(signal['close'][i] * shareNo)
                signPriceSell.append(np.nan)
                flag = 1
                cashbalance = cashbalance - signal['close'][i] * shareNo
            else:
                borrowed+= signal['close'][i] * shareNo - cashbalance
                cashbalance = 0
                signPriceBuy.append(signal['close'][i] * shareNo)
                signPriceSell.append(np.nan)
                flag = 1
        # If RSI_14 > 50 Then Sell
        elif signal[col1][i] == 0 and signal[col2][i] == 1 and flag != 0:
            signPriceSell.append(signal['close'][i] * 10000)
            ...
            signPriceBuy.append(np.nan)
    if flag == 1:
        signPriceSell.append(signal['close'][len(signal)-1] * 10000)
    ...
    return (signPriceBuy, signPriceSell, cashbalance, borrowed)
...
def OBV_buy_sell(signal, col1, col2, cashbalance):
    signPriceBuy = []
    signPriceSell = []
    flag = -1
    shareNo = 0
    ...
    for i in range(0, len(signal)):
        ...
        if signal[col1][i] > signal[col2][i] and flag != 1:
            shareNo = abs((signal[col1][i]-signal[col2][i]) / signal[col2][i] * 10000)
            signPriceBuy.append(signal['close'][i] * shareNo)
            print('share number : ', shareNo)
            signPriceSell.append(np.nan)
            flag = 1
        # If OBV < OBV_EMA Then Sell
        elif signal[col1][i] < signal[col2][i] and flag != 0:
            signPriceSell.append(signal['close'][i] * shareNo)
            signPriceBuy.append(np.nan)
            flag = 0
        else:
            signPriceSell.append(np.nan)
            signPriceBuy.append(np.nan)
    if flag == 1:
        signPriceSell.append(signal['close'][len(signal)-1] * shareNo)
...
for i, spl in enumerate(strat_params_list):
    ...
    rsi_buy_price, rsi_sell_price, cashbalance, borrowed = rsi_buy_sell(data, RSIflag.columns[1], RSIflag.columns[2], cashbalance, borrowed)
    obv_buy_price, obv_sell_price = OBV_buy_sell(OBVcalculation(data), 'OBV', 'OBV_EMA', cashbalance)
...
def leverage(buy_price, sell_price, interest, leverage, cashbalance):
    borrowed = 0
...
cashbalance, borrowed = leverage([1,2,3,4,5,6,7],[2,3,4,4,3,2,1], 0.2, 2, 10)
